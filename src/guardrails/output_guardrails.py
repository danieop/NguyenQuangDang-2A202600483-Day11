...
from agents.agent import _build_shopaikey_model
...
def _init_judge():
    ...
    if safety_judge_agent is None:
        # The judge is a separate model-backed agent so output review stays independent
        # from the main assistant and can catch leaked content that the assistant itself misses.
        safety_judge_agent = llm_agent.LlmAgent(
...
class OutputGuardrailPlugin(base_plugin.BasePlugin):
    ...
    async def after_model_callback(
        ...
        *,
        ...
        # Redact secrets and PII before the text reaches the user.
        ...
        # Use an independent judge to catch unsafe content that a regex filter can miss.
                ...
                llm_response.content = types.Content(
                    ...
                    parts=[
                        types.Part.from_text(
                            text="I’m unable to provide that response because it may expose sensitive information or unsafe content."
                        )
                    ],
                )
